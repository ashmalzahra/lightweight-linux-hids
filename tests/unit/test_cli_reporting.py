...
def test_report_scan_hides_event_details_but_shows_alert(capsys) -> None:
    event = Event(
    ...
    assert "Activity: 1 event(s) recorded; 1 alert(s)." in output


def test_verbose_report_shows_source_specific_event_details(capsys) -> None:
    file_event = Event(
    ...
    report_scan(
        [file_event, authentication_event],
        [],
        verbose_events=True,
    )
    ...
    assert "Activity: 2 event(s) recorded; 0 alert(s)." in output
